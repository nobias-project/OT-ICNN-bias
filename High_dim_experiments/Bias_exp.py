...
# -*- coding: utf-8 -*-
"""
Created on Mon Jul  4 16:17:22 2022
...
parser = argparse.ArgumentParser(description='PyTorch Bias Experiment')
...
parser.add_argument('--NUM_NEURON', type=int, default=256, help='number of neurons per layer')
...
X_data = mu + torch.randn(20000, args.INPUT_DIM)
X_data = torch.cat((X_data, torch.randn(40000, args.INPUT_DIM)))
...
# 
y = torch.randn(30, args.INPUT_DIM, requires_grad=True)
# 
x = mu + torch.randn(10, args.INPUT_DIM)
x = torch.cat( (x, torch.randn(20, args.INPUT_DIM)))
if args.cuda:
...
x_plot = np.arange(-5, 15)
y_plot = np.arange(-5, 15)
    
xy_plot = np.array([[a,b] for a,b in itertools.product(x_plot,y_plot)]).reshape((20,20,2))
z = convex_f(torch.from_numpy(xy_plot).cuda().float()).detach().cpu().numpy()

contours = plt.contour(x_plot,y_plot,z.reshape((20,20)), levels = 17)
plt.clabel(contours, inline = 1, fontsize = 10)
plt.savefig(results_save_path+'/contours.png')
plt.show()
