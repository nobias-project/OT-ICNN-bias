...
def convex_fn_inverse_constraint_loss_y_side(convex_f, convex_g, y):
    ...
    grad_f_grad_g_of_y = torch.autograd.grad(f_grad_g_of_y,
                                             grad_g_of_y,
                                             create_graph=True)[0]
...
def convex_fn_inverse_constraint_loss_x_side(convex_f, convex_g, real_data):
    ...
    grad_g_grad_f_of_x = torch.autograd.grad(g_grad_f_of_x,
                                             grad_f_of_x,
                                             create_graph=True)[0]
...
def inequality_young_fenchel_loss(convex_f, convex_g, real_data, y):
    ...
    return torch.mean(
        (torch.clamp((torch.bmm(real_data.view(-1, 1, size_of_y),
                                y.view(-1, size_of_y, 1)).reshape(-1, 1)
                      - convex_f(real_data)
                      - convex_g(y)), min=0)
         )**2
        )
...
def equality_young_fenchel_loss(grad_g_of_y,
                                f_grad_g_y,
                                real_data,
                                y,
                                convex_g):
    ...
    y_transport_loss = torch.mean(
                       (f_grad_g_y +
                        convex_g(y) -
                        torch.bmm(grad_g_of_y.view(-1, 1, size_of_y),
                                  y.view(-1, size_of_y, 1)).reshape(-1, 1)
                        )**2
                       )

    # # This is for x-transport loss.
    # This doesn't completely make sense since 'x' doesn't have a density
    # x = Variable(real_data, requires_grad=True)
    ...
    # x_transport_loss = torch.mean((convex_g(grad_f_of_x) + convex_f(x) -
    # torch.bmm(grad_f_of_x.view(-1, 1, size_of_y),
    # x.view(-1, size_of_y, 1)).reshape(-1, 1) )**2)
